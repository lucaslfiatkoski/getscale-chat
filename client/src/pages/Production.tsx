...
export default function Production({ nickname, isAdmin }: ProductionProps) {
  ...
  useEffect(() => {
    ...
    socketInstance.on("production:new", (newOrder: Order) => {
      console.log("[Production] Novo pedido recebido do servidor:", newOrder);
      setOrders(prev => {
        // Evitar duplicação se já foi adicionado localmente
        const exists = prev.some(o => o.id === newOrder.id);
        if (exists) {
          console.log("[Production] Pedido já existe localmente, ignorando duplicação");
          return prev;
        }
        const updated = [...prev, newOrder];
        console.log("[Production] Lista atualizada:", updated.length, "pedidos");
        return updated;
      });
    });
  ...
  const handleCreateOrder = () => {
    ...
    if (socket) {
      ...
      // Adicionar localmente de forma otimista
      setOrders(prev => {
        const updated = [...prev, newOrder];
        console.log("[Production] Adicionado localmente (otimista):", updated.length, "pedidos");
        return updated;
      });
      socket.emit("production:create", newOrder);
      toast.success(`Pedido criado: ${customerName}`);
    } else {
  ...
  const handleDeleteAllOrders = () => {
    if (!isAdmin) {
      toast.error("Apenas administradores podem deletar todos os pedidos");
      return;
    }

    if (!confirm("ATENÇÃO: Tem certeza que deseja excluir TODOS os pedidos? Esta ação não pode ser desfeita!")) return;

    if (socket) {
      // Deletar cada pedido individualmente
      orders.forEach(order => {
        socket.emit("production:delete", order.id);
      });
    }
        ...
        <ScrollArea className="h-full">
          ...
          {sortedOrders.length === 0 ? (
            ...
            sortedOrders.map(order => {
              ...
              return (
                <Card key={order.id} className={`p-3 ${order.status.enviado ? 'bg-green-50' : 'bg-white'}`}>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <h3 className="text-base font-bold">{order.customerName}</h3>
                    </div>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                  ...
                  {isExpanded && (
                    <div className="mb-3 pb-3 border-b">
                      <p className="text-xs text-gray-600 mb-2">Data: {formatDate(order.orderDate)}</p>
                      <div className="space-y-2">
                        <p className="text-xs font-semibold text-gray-700 mb-1">Itens:</p>
                        <div className="space-y-1">
                          {order.items.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between bg-gradient-to-r from-blue-50 to-cyan-50 px-2 py-1.5 rounded-lg border border-blue-200 w-full">
                              <div className="flex-1 space-y-1">
                              ...
                              <span className="bg-blue-600 text-white px-2 py-0.5 rounded-full font-bold text-xs whitespace-nowrap ml-2">Qtd: {item.quantity}</span>
                            </div>
                          ))}
                        </div>
                      </div>
