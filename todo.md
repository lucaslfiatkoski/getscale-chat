...
## Bugs Reportados
- [x] Pedidos não aparecem após criação - CORRIGIDO com atualização otimista + logs de debug
- [x] Login é perdido ao recarregar página (Ctrl+F5) - persistido no localStorage

## Nova Melhoria Solicitada
- [x] Ajustar layout do Controle de Produção para telas 1920x1080 (adicionar scroll e otimizar espaço)

## Nova Funcionalidade
- [x] Adicionar 20 pedidos de exemplo pré-carregados para testes

## Nova Funcionalidade Crítica
- [x] Implementar persistência em disco (JSON) para mensagens e pedidos
- [x] Remover pedidos de teste (deixar vazio inicialmente)

## Bug Reportado
- [x] Página do chat não se redimensiona corretamente com a janela (parte está cortando)

## Bug Reportado
- [x] Remover pedidos que aparecem inicialmente na aba Controle de Produção (servidor inicia vazio)

## Nova Funcionalidade Solicitada
- [x] Adicionar botão para deletar todos os pedidos (apenas admin)

## Novas Funcionalidades Solicitadas
- [x] Notificações sonoras quando receber mensagens em salas inativas
- [x] Histórico de mensagens com paginação/scroll infinito
